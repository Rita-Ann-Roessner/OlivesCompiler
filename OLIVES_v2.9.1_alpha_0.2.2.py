...
print("OLIVES_v2.9.1_alpha_0.2.2.py was written by Kasper Busk Pedersen, June 28th 2024.")
print("If you use this script in your work, please cite Pedersen et al. 2024, https://doi.org/10.1021/acs.jctc.4c00553")
...
hbond_dict = {"GLY":{"C":[[0,0],["None","None"]],"CA":[[0,0],["None","None"]],"O":[[1,0],["AMA","None"]],"N":[[0,1],["None","AMD"]]},
              ...
              "CYP":{"C":[[0,0],["None","None"]],"CA":[[0,0],["None","None"]],"O":[[1,0],["AMA","None"]],"N":[[0,1],["None","AMD"]],"SC1":[[1,1],["KEA","HYD"]],"C1":[[0,0],["None","None"]],"C2":[[0,0],["None","None"]],"C3":[[0,0],["None","None"]],"C4":[[0,0],["None","None"]]}, #Palmitoylated cysteine
              ...
              "HYP":{"C":[[0,0],["None","None"]],"CA":[[0,0],["None","None"]],"O":[[1,0],["AMA","None"]],"N":[[0,0],["None","None"]],"SC1":[[1,1],["KEA","HYD"]]}, #Hydroxyproline
              ...
              "ARG":{"C":[[0,0],["None","None"]],"CA":[[0,0],["None","None"]],"O":[[1,0],["AMA","None"]],"N":[[0,1],["None","AMD"]],"SC1":[[0,0],["None","None"]],"SC2":[[0,1],["None","IMD"]],"SC3":[[0,1],["None","IMD"]],"SC4":[[0,1],["None","IMD"]]}, 
...
special_residues = {"GLN", "ASN", "LYS", "ASP", "GLU"} #HIS and ARG not included since they are mapped with higher resolution, allowing multiple HBs per side chain in the first matching round 
...
def user_input():
    ...
    parser.add_argument('-c', help='File containing the coarse-grained structure(s) of the protein in pdb format. Multistate mode is enabled by providing multiple conformations, e.g.: conf1.pdb,conf2.pdb,conf3.pdb. The conformations must have the exact same topology. "--unique_pair_scaling" must be specified in multistate mode')
    parser.add_argument('-i', help='File containing the protein topology e.g. molecule_0.itp from martinize2.')
    parser.add_argument('--ss_cutoff', type=float, default=0.36, help='Cutoff distance for generation of the secondary network (and quaternary network between backbone beads). Do not decrease, and only incease only if an important HB pair is slightly beyond the cutoff (default=0.36).')
    parser.add_argument('--ts_cutoff', type=float, default=0.55, help='Cutoff distance for generation of the tertiary network (and quaternary netowrk between backbone-sidecahin and sidechain-sidechain pairs). Do not decrease, and only increase if an important HB pair is slightly beyond the cutoff (default=0.55).')
    parser.add_argument('--ss_scaling', type=float, default=0.3, help='Scaling of the secondary structure OLIVES bonds. Recommended range [0.2-0.4] (default=0.3).')
    parser.add_argument('--ts_scaling', type=float, default=0.8, help='Scaling of the tertiary structure OLIVES bonds. Recommended range [0.7-1.0] (default=0.8).')
    parser.add_argument('--qs_scaling', type=str, default='{}', help='Scaling of the quaternary bonds between .pdb chain indices (chains must have different names in the pdb file). Input format is a dictionary of chain index tuples with associated scaling value. E.g. in a protein with chain A,B,C the input: {(0,1):0.7,(0,2):0.5,(1,2):0.2} will scale quaternary bonds between chain A and B by 0.7, A and C by 0.5, and B and C by 0.2. The tuple must be sorted like in the example.')
    parser.add_argument('--unique_pair_scaling', type=str, default="1.0", help='Multistate mode: Scaling of the unique contacts of each conformation, provided as a comma-separated sting e.g. "0.5,0.75" will scale unique contacts in conf 1 by 0.5 and conf 2 by 0.75. Networks are still scaled by the "_scaling" settings, so default values for these are recommended. Shared contacts between conformations have their r_min distances averaged. ')
...
def detect_cys_bridges(cut_pairs, cut_dists, filters):
...
def should_ignore_cys(pair, cys_bridges, a_ndx_to_res_ndx):
    ...
    for b in cys_bridges:
        if a_ndx_to_res_ndx[pair[0]] == b[0] and a_ndx_to_res_ndx[pair[1]] == b[1]:
            return True
        if a_ndx_to_res_ndx[pair[0]] == b[1] and a_ndx_to_res_ndx[pair[1]] == b[0]:
            return True
    return False
...
def perform_max_weight_matching(weighted_pairs,max_cardinality):
    # Performs maximum weight matching on the given pairs.
    graph = nx.Graph()
    graph.add_weighted_edges_from(weighted_pairs)
    matched_network = nx.max_weight_matching(graph, maxcardinality=max_cardinality)
...
def get_special_case_pairs(remaining_pairs, flat_matched_pairs, a_ndx_to_res_name, a_ndx_to_bead_name, special_residues):
    ...
    for p in remaining_pairs:
        if a_ndx_to_res_name[p[0]] in special_residues:
            if a_ndx_to_bead_name[p[0]] == 'O' or a_ndx_to_bead_name[p[0]] == 'N':
                continue #only select the sidechain beads
            if p[1] in flat_matched_pairs:
                if a_ndx_to_bead_name[p[1]] == 'O' or a_ndx_to_bead_name[p[1]] == 'N':
                    continue # # if the backbone bead is already bonded in the first round, do not consider it again
...
def knowledge_based_checks(cut_pairs, cut_dists, filters):
    ...
    a_ndx_to_bead_name, a_ndx_to_res_name, a_ndx_to_res_ndx = filters[:3]
    cys_bridges = detect_cys_bridges(cut_pairs, cut_dists, filters)
    ...
    for i, pair in enumerate(cut_pairs):
        ...
        if (a_ndx_to_bead_name[pair[0]] == 'O' and a_ndx_to_bead_name[pair[1]] == 'O') or (a_ndx_to_bead_name[pair[0]] == 'N' and a_ndx_to_bead_name[pair[1]] == 'N'):
            continue
...
def match_secondary_network(checked_pairs_dists_energies, checked_pairs):
    ...
    matched_pairs_pass1 = perform_max_weight_matching(weighted_pairs_pass1,max_cardinality=False)
    flat_matched_pairs_pass1 = [item for pair in matched_pairs_pass1 for item in pair]
    
    remaining_pairs1 = []
    for pair in list(set(checked_pairs) - set(matched_pairs_pass1)):
        if pair[0] in flat_matched_pairs_pass1 or pair[1] in flat_matched_pairs_pass1:
            continue
    ...
    matched_pairs_pass2 = perform_max_weight_matching(weighted_pairs_pass2,max_cardinality=True)

    matched_secondary_pairs = sorted(matched_pairs_pass1 + matched_pairs_pass2)
...
def combine_and_format_potentials(pairs_dict,networktype,unique_pairs,unique_pair_scaling,energy_scaling,quaternary_energy_scaling,filters):
    ...
    for pair, dist_energy in pairs_dict.items():
        ...
        if networktype == "secondary":
            LJ_pairs.append([str(pair[0]+1),str(pair[1]+1),f"{LJ_function_type}","{:.10f}".format(mean_dist/np.power(2,1/6)),"{:.10f}".format(HB_energy)]) #Here we add back in the (unscaled) P2-P2 LJ energy that was exlcuded, to be consistent with the tertiary network
...
def generate_output(args, itp_CG, secondary_LJ_potentials, tertiary_LJ_potentials, 
...
def main():
    ...
    #Write output files
    generate_output(args, itp_CG, secondary_LJ_potentials, tertiary_LJ_potentials, secondary_harm_potentials, tertiary_harm_potentials, info_secondary_pairs, info_tertiary_pairs, secondary_quaternary_pair_info, tertiary_quaternary_pair_info, quaternary_energy_scaling)

    if not args.silent:
        print('Remember to use "gmx mdrun --noddcheck -rdd 2.0" to avoid domain decomposition warnings if OLIVES bonds gets too long.')
        print('"-noddcheck" turns off a GROMACS domain decomposition (DD) error for pairs that become too long relative to the length of a domain (set by -rdd), abruptly ending your run.')
        ...
        print("One can opt to restrain the secondary structure using the martinize2 -dssp flag at the cost of overstabilized secondary structure. The secondary network energies might need to be downscaled or removed to avoid instabilities.")
